...
trait Reaction
{
    ...
    public function reaction($post_id, $type="LIKE")
    {
        ...
        if (isset($selector[$type])) {
            $a = explode('<a href=', $this->get_reaction_list($this->get_page(((substr($post_id, 0, 8)=="https://" or substr($post_id, 0, 7)=="http://") ? $post_id : self::FBURL.'/'.$post_id), null, null)));
            $reaction_list = array();
            ...
            return $this->curl(substr($reaction_list[$selector[$type]], 1), null, array(CURLOPT_REFERER=>$this->curl_info['url']));
        } else {
